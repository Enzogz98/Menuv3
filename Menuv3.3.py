...
class PDFApp(tk.Tk):
    ...
    def migrate_to_sheets(self):
            if self.generated_excel_files_directory:
                ...
                if os.path.exists(filtered_dir) and any(f.endswith('.xlsx') for f in os.listdir(filtered_dir)):
                    ...
                    self.run_macro(self.generated_file_path, "CloverQR")
    ...
    def combine_and_save_by_tarjeta(self, directory, output_dir):
        nombres_tarjetas = ["VISA DEBIT", "VISA", "MASTERCARD DEBIT", "MASTERCARD", "NaranjaX", "ARGENCARD", "Detalles", "AMEX", "MAESTRO", "Resumen", "Pagos"]
        ...
        # Iterar directamente sobre los archivos en el directorio
        ...
        # Opcional: eliminar archivos originales si es necesario
    ...
    "Motivo de rechazo", "Número de Terminal", "Número de referencia", "Fecha de liquidación", "Numero de cuotas"
        ...
        headers_qr=['FECHA DE OPERACIÓN', 'N° COMERCIO', 'TOTAL BRUTO', 'N° TRANSACCIÓN', 'ENTIDAD PAGADORA', 'BILLETERA', 'TIPO DE OPERACIÓN']
        ...
        try:
            ...
            if 'Resumen' in file_path:
                ...
                if data.iloc[:, 0].isin(['Plan cuota', 'Venta ctdo', 'Contr ctdo', 'Devol ctdo', 'Contr cuo', 'Devol cuo']).any():
                    #repres cuo, ctdo puede chotearse
                    filtered_data = data[data.iloc[:, 0].isin(['Plan cuota', 'Venta ctdo','Contr ctdo', 'Devol ctdo', 'Contr cuo', 'Devol cuo'])]
                    return filtered_data
                else:
